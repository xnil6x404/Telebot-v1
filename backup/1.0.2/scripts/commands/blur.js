...
module.exports = {
  ...
  ownerOnly: false,
  category: 'Fun',
  description: 'Apply a blur effect to an image or your profile picture',
  guide: 'Reply to an image with /blur or use /blur to apply a blur effect to your profile picture.',
  execute: async (bot, msg) => {
    ...
    const apiUrl = `https://api.elianabot.xyz/tools/blur.php?url=${encodeURIComponent(imageUrl)}`;

    try {
      const response = await axios.get(apiUrl, { responseType: 'arraybuffer' });
      const imageBuffer = Buffer.from(response.data, 'binary');

      // Send the blurred image received from the API
      await bot.sendPhoto(chatId, imageBuffer, { caption: 'Here is your blurred image!' });
    } catch (error) {
