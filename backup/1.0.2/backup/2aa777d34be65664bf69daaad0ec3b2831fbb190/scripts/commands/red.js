...
module.exports = {
  ...
  ownerOnly: false,
  category: 'Fun',
  description: 'Apply a red filter to an image or your profile picture',
  guide: 'Reply to an image with /red or use /red to apply a red filter to your profile picture.',
  execute: async (bot, msg) => {
    ...
    const apiUrl = `https://api.elianabot.xyz/tools/red.php?image=${encodeURIComponent(imageUrl)}`;

    try {
      const response = await axios.get(apiUrl, { responseType: 'arraybuffer' });
      const imageBuffer = Buffer.from(response.data, 'binary');

      // Send the red-filtered image received from the API
      await bot.sendPhoto(chatId, imageBuffer, { caption: 'Here is your red-filtered image!' });
    } catch (error) {
      console.error('Error in red command:', error);
